...
import { AnnotationModal, PDFViewer, PhotoGallery, PhotoMenu, ProfileModal } from "./pv/components/index.js";
import { I } from "./pv/icons.jsx";
import { CURRENT_USER, SAMPLE_PVS, SURFACE_FIELDS, RELEVES_FIELDS, POINT_FIELDS } from "./pv/schema.js";
import {
  createPdfDataFromPV,
  downloadPDF,
  pickImages,
...
import {
  ...
  useFormManagement,
  usePVOperations,
} from "./pv/hooks/index.js";
import {
  renderStep1,
  renderStep2,
  renderStep3,
  renderStep4,
} from "./pv/components/StepRenderers.jsx";
import { renderStep5 } from "./pv/components/ParticipantStepRenderer.jsx";
import { renderReserveCard, renderReserveContent, renderReserveList } from "./pv/components/ReserveRenderer.jsx";
import { renderSplashScreen, renderHomeScreen } from "./pv/components/ScreenRenderers.jsx";
import SuccessScreen from "./pv/components/SuccessScreen.jsx";

export default function App() {
  // State management
  const appState = useAppState();
  const { toasts, push: toast } = useToast();
  const [pvList, setPvList] = React.useState(SAMPLE_PVS);
  ...
  const nextStep = () => {
    if (appState.step === 2) {
      ...
      SURFACE_FIELDS.forEach(([key]) => {
        ...
        const photos = formMgmt.etatSurface[key + "Photos"] || [];
        const comment = (formMgmt.etatSurface[key + "Comment"] || "").trim();
  ...
  const renderStep = () => {
    ...
    if (appState.step === 1) {
      return renderStep1(
        ...
        reserveManagement.reserves,
        () => { reserveManagement.openAddRes(); appState.setReserveScreen("add"); },
        (reserve, index, onEdit, onDelete) => renderReserveCard(reserve, index, onEdit, onDelete, appState.setLightbox),
        reserveManagement.deleteRes,
        (reserve) => { reserveManagement.openEditRes(reserve); appState.setReserveScreen("add"); },
    ...
    if (appState.step === 2) {
      return renderStep2(
        ...
        async (key) => {
          ...
          const current = formMgmt.etatSurface[key + "Photos"] || [];
        ...
        (key, photoId) => {
          formMgmt.setEtatSurface((curr) => ({
            ...curr,
            [key + "Photos"]: (curr[key + "Photos"] || []).filter((p) => p.id !== photoId),
          }));
        },
        (menu) => appState.setPhotoMenu(menu),
  ...
  if (appState.screen === "success") {
    ...
    const savedPVObj = savedResult?.pv;
    return (
      <div className="shell">
        <SuccessScreen
          ...
          onShareEmail={() => {
            ...
            if (email) window.location.href = `mailto:${email}?subject=PV ${savedResult?.ref}`;
          }}
        />
      </div>
    );
      ...
      <div className="shell">
        <div className="scr">
          ...
          {showNavBar && appState.step < 5 && (
            <div
              style={{
                ...
                display: "flex",
                gap: 10,
                justifyContent: appState.step === 1 ? "flex-end" : "space-between",
              }}
            >
              {appState.step > 1 && (
          ...
          {showNavBar && appState.step === 5 && (
            ...
            >
              <button className="btn-gray" onClick={prevStep} style={{ minWidth: 0 }}>
                <I.Back /> Précédent
              </button>
              <button className="btn-red" onClick={savePV} style={{ flex: 1 }}>
                <I.Check /> Enregistrer
              </button>
            </div>
          )}
        ...
        <div className="modal-bg" onClick={() => appState.setShowQuit(false)}>
          <div className="modal-sheet" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="modal-body">
              Souhaitez-vous vraiment quitter ce formulaire ? Cette action est irréversible pour la session en cours.
            </div>
            <div className="modal-actions">
              <button
                className="btn-red btn-full"
                onClick={() => {
                  appState.setShowQuit(false);
                  appState.setScreen("home");
                  appState.setStep(1);
                  appState.setReserveScreen("list");
                  formMgmt.setEditPvId(null);
              ...
              <button className="btn-gray btn-full" onClick={() => appState.setShowQuit(false)}>
                Rester
              </button>
            </div>
          </div>
          ...
          onPhotoClick={(photo) => {
            ...
            appState.setPhotoMenu({
              ...
              onUpdate: (newUrl) => {
                ...
                appState.setGallery((prev) => prev ? ({
                  ...prev,
                  photos: prev.photos.map((p) => p.id === photo.id ? { ...p, url: newUrl } : p),
                }) : null);
              },
            });
          }}
