...
import { pickImages, uid } from "../helpers.js";

export const useReserveManagement = (toast) => {
  const [reserves, setReserves] = useState([]);
  const [rForm, setRForm] = useState({ localisation: "", detail: "", photos: [], localisationPhoto: null });
  ...
  const addPhotoToResCard = async (id) => {
    ...
    setReserves((prev) =>
      prev.map((reserve) => (reserve.id === id ? { ...reserve, photos: [...(reserve.photos || []), ...images] } : reserve))
    );
  ...
  const openAddRes = () => {
    setRForm({ localisation: "", detail: "", photos: [], localisationPhoto: null });
    setDetailErr(false);
    setEditResId(null);
  };
  ...
  const updateRFormLocalisationPhotoUrl = (newUrl) => {
    setRForm((current) => ({
      ...current,
      localisationPhoto: { ...current.localisationPhoto, url: newUrl },
    }));
  };
