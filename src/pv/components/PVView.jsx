...
import { SURFACE_FIELDS, RELEVES_FIELDS, POINT_FIELDS } from "../schema.js";
...
const StatusRow = ({ label, value, last }) => (
  <div style={{
    display: "flex", justifyContent: "space-between", alignItems: "center",
    padding: "11px 16px",
    borderBottom: last ? "none" : "1px solid #f3f4f6",
...
const Card = ({ title, children, collapsible = false, defaultOpen = true }) => {
  ...
  return (
    <div style={{
      ...
      {title && (
        ...
        >
          <span style={{ fontSize: 11, fontWeight: 800, color: "#1a1a1a", textTransform: "uppercase", letterSpacing: 1.2 }}>
            {title}
          </span>
          {collapsible && (
            <svg
...
function PhotoStrip({ photos, borderColor }) {
        ...
        {photos.map((photo) => (
          <img
            ...
            style={{
              ...
              objectFit: "cover",
              cursor: "pointer",
              border: borderColor ? `1.5px solid ${borderColor}` : "1.5px solid #e5e7eb",
              flexShrink: 0,
            }}
          />
        ))}
...
export default function PVView({ pv, onBack, onPDF, onEdit }) {
        ...
        <div style={{ display: "flex", gap: 8, marginLeft: "auto" }}>
          {versions.length > 0 && (
            <button
              ...
              style={{ background: "#f0f0f0", border: "none", borderRadius: "50%", width: 38, height: 38, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#555", position: "relative", flexShrink: 0 }}
            >
              <I.History />
              <span style={{ position: "absolute", top: -3, right: -3, background: "#D32F2F", color: "#fff", borderRadius: 50, fontSize: 9, fontWeight: 800, padding: "1px 5px", lineHeight: 1.4 }}>
                {versions.length}
              </span>
            </button>
          )}
          <button
            onClick={onEdit}
            style={{ background: "#fff0f0", border: "none", borderRadius: "50%", width: 38, height: 38, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#D32F2F", flexShrink: 0 }}
            title="Modifier"
          >
            <I.Annotate />
          </button>
      ...
      <div className="cnt fade-in">
          ...
          <Card title={`Réserves (${reserves.length})`}>
            {reserves.map((reserve, index) => (
              <div key={reserve.id || index} style={{ padding: "12px 16px", borderBottom: index < reserves.length - 1 ? "1px solid #f3f4f6" : "none" }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: "#D32F2F", textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 4 }}>
                  Réserve #{String(index + 1).padStart(2, "0")}
                </div>
                <div style={{ fontSize: 14, fontWeight: 600, color: "#1a1a1a", marginBottom: reserve.detail ? 4 : 0 }}>
                  {reserve.localisation || "—"}
                </div>
                {reserve.detail && (
                  <div style={{ fontSize: 13, color: "#6b7280" }}>{reserve.detail}</div>
                )}
                {(reserve.localisationPhoto || (reserve.photos || []).length > 0) && (
                  <div style={{ marginTop: 10 }}>
                    ...
                    {(reserve.photos || []).length > 0 && (
                      <PhotoStrip photos={reserve.photos} />
                    )}
                  </div>
                )}
              </div>
        ...
        <Card title="État de surface">
          {SURFACE_FIELDS.map(([key, label], index) => (
            <div key={key}>
              <StatusRow label={label} value={etatSurface[key]} last={index === SURFACE_FIELDS.length - 1 && etatSurface[key] !== "Non Conforme"} />
              {etatSurface[key] === "Non Conforme" && (
                <div style={{ padding: "0 16px 12px", background: "#fff8f8" }}>
                  {(etatSurface[key + "Photos"] || []).length > 0 && (
                    <div style={{ marginBottom: 10 }}>
                      <PhotoStrip photos={etatSurface[key + "Photos"]} borderColor="#fdd" />
                    </div>
                  )}
          ...
          {participants.map((participant, index) => {
            ...
            return (
              <div key={participant.id || index} style={{ padding: "12px 16px", borderBottom: index < participants.length - 1 ? "1px solid #f3f4f6" : "none" }}>
                ...
                {!isSMAC && participant.titre && (
                  <div style={{ fontSize: 13, color: "#9ca3af", marginBottom: 2 }}>{participant.titre}</div>
                )}
