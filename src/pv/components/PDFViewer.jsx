...
import { POINT_FIELDS, RELEVES_FIELDS, SURFACE_FIELDS } from "../schema.js";
import { downloadPDF } from "../helpers.js";

const Badge = ({ value = "SO" }) => {
...
export default function PDFViewer({ pvData, onClose }) {
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                        {reserve.photos.map((photo) => (
                          <img
                            ...
                            src={photo.url}
                            alt=""
                            style={{ width: "100%", borderRadius: 10, border: "2px solid #e0dcd8", objectFit: "cover", display: "block" }}
                          />
                        ))}
                      </div>
                    </div>
                  ...
                  <div style={{ padding: "10px 14px 14px", background: "#fff8f8", borderBottom: "1px solid #f5f0eb" }}>
                    {(etatSurface[key + "Photos"] || []).length > 0 && (
                      <div style={{ marginBottom: etatSurface[key + "Comment"] ? 10 : 0 }}>
                        <div style={{ fontSize: 11, fontWeight: 700, color: "#D32F2F", marginBottom: 8, textTransform: "uppercase", letterSpacing: 0.5 }}>
                          Photos ({etatSurface[key + "Photos"].length})
                        </div>
                        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                          {etatSurface[key + "Photos"].map((photo) => (
                            <img
                              key={photo.id}
                              src={photo.url}
                              alt=""
                              style={{ width: "100%", borderRadius: 10, border: "2px solid #D32F2F", objectFit: "cover", display: "block" }}
                            />
                          ))}
                        </div>
                      </div>
            ...
            {participants.map((participant, index) => (
              <div key={participant.id || index} className="pdf-sig-block">
                <div className="pdf-sig-name">
                  ...
                  <span
                    className="pdf-sig-status"
                    style={{
                      background: participant.signed ? "#e8f5e9" : "#f5f5f5",
                      color: participant.signed ? "#1a7a3f" : "#bbb",
                    }}
                  >
