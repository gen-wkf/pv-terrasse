...
export const renderReserveList = (reserves, onBack, onAdd, onEdit, onDelete, onLightbox) => (
  <div className="cnt fade-in">
    ...
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
      ...
      <div style={{ fontSize: 13, fontWeight: 700, color: "#6b7280" }}>
        {String(reserves.length).padStart(2, "0")} / 08
      </div>
    </div>
    ...
    {reserves.map((reserve, index) => (
      <div
        ...
        style={{ background: "#fff", borderRadius: 14, border: "1.5px solid #f0ece8", marginBottom: 12, overflow: "hidden", boxShadow: "0 2px 6px rgba(0,0,0,0.04)", display: "flex", cursor: "pointer" }}
      >
        {/* Red left bar */}
        ...
        <div style={{ flex: 1, padding: "12px 14px" }}>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 10, fontWeight: 800, color: "#D32F2F", textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 3 }}>
              ...
              {reserve.detail && (
                <div style={{ fontSize: 12, color: "#6b7280", marginTop: 2 }}>{reserve.detail}</div>
              )}
            </div>
            <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
              <button
                onClick={(e) => { e.stopPropagation(); onEdit(reserve); }}
                style={{ width: 34, height: 34, borderRadius: 8, background: "#f3f4f6", border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#6b7280" }}
              >
                <I.Annotate />
              </button>
              <button
                onClick={(e) => { e.stopPropagation(); onDelete(reserve.id); }}
                style={{ width: 34, height: 34, borderRadius: 8, background: "#fff0f0", border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#D32F2F" }}
              >
                <I.Trash />
              </button>
            </div>
          ...
          {((reserve.photos || []).length > 0 || reserve.localisationPhoto) && (
            <div style={{ display: "flex", gap: 6, marginTop: 10, flexWrap: "wrap" }}>
              {reserve.localisationPhoto && (
                <img
                  src={reserve.localisationPhoto.url}
                  alt="Loc"
                  onClick={(e) => { e.stopPropagation(); onLightbox(reserve.localisationPhoto.url); }}
                  style={{ width: 60, height: 60, borderRadius: 8, objectFit: "cover", border: "2px solid #D32F2F", cursor: "pointer", flexShrink: 0 }}
                />
              )}
              {(reserve.photos || []).map((photo) => (
                <img
                  ...
                  src={photo.url}
                  alt=""
                  onClick={(e) => { e.stopPropagation(); onLightbox(photo.url); }}
                  style={{ width: 60, height: 60, borderRadius: 8, objectFit: "cover", border: "1.5px solid #e5e7eb", cursor: "pointer", flexShrink: 0 }}
                />
              ))}
            </div>
          )}
    ...
    {reserves.length < 8 && (
      <button
        ...
        style={{
          ...
          border: "1.5px dashed #D32F2F", borderRadius: 14,
...
export const renderReserveCard = (reserve, index, onEdit, onDelete, onLightbox) => (
  <div key={reserve.id} className="reserve-card-item">
    <div className="rc-row">
      ...
      <div className="rc-acts">
        <button className="card-ico-btn" onClick={() => onEdit(reserve)}>
          <I.Pencil />
        </button>
        <button className="card-ico-btn" onClick={() => onDelete(reserve.id)}>
          <I.Trash />
        </button>
      </div>
    </div>
    <div className="rc-photos">
      {reserve.localisationPhoto && (
        <img
          ...
          onClick={() => onLightbox(reserve.localisationPhoto.url)}
          title="Photo de localisation"
        />
      )}
      {(reserve.photos || []).map((photo) => (
...
export const renderReserveContent = (
  ...
  onReplacePhoto,
  onUpdatePhotoUrl,
  onPickLocalisationPhoto,
  onRemoveLocalisationPhoto,
  ...
  <div className="cnt fade-in">
      ...
      {rForm.photos.length === 0 ? (
        <div
          ...
          style={{
            ...
            background: "#fafafa", padding: "22px 16px",
            display: "flex", flexDirection: "column", alignItems: "center", gap: 8,
            cursor: "pointer",
          }}
        >
          <CameraIcon />
          <span style={{ fontSize: 12, fontWeight: 800, color: "#D32F2F", letterSpacing: 0.8, textTransform: "uppercase" }}>
            Prendre / Importer une photo
          </span>
        </div>
      ) : (
        <div className="photo-grid">
          {rForm.photos.map((photo) => (
            <div key={photo.id} className="ph-wrap">
              <img
                ...
                onClick={() => onPhotoMenu({
                  photo,
                  onReplace: () => onReplacePhoto(photo.id),
                  onUpdate: (url) => onUpdatePhotoUrl(photo.id, url),
                })}
              />
          ...
          {rForm.photos.length < 8 && (
            <div className="ph-add-tile" onClick={onAddPhoto} title="Ajouter une image">
              <I.PlusCircle />
            </div>
          )}
        </div>
    ...
    <div className="field-wrap">
      ...
      <input
        ...
        value={rForm.localisation}
        onChange={(event) => onChangeLocalisation(event.target.value)}
        autoComplete="off"
        autoCorrect="off"
        spellCheck="false"
      />
    ...
    <div style={{ marginBottom: 16 }}>
      {!rForm.localisationPhoto ? (
        <div
          onClick={onPickLocalisationPhoto}
          style={{
            border: "1.5px dashed #e5e7eb", borderRadius: 12,
            background: "#fafafa", padding: "22px 16px",
            display: "flex", flexDirection: "column", alignItems: "center", gap: 8,
    ...
    <div className="field-wrap">
      <label className="lbl">
        Détail de la réserve et actions à mener <span className="req-star">*</span>
      </label>
      <textarea
        className={`inp${detailErr ? " inp-err" : ""}`}
    ...
    {/* Sauvegarder */}
    <button className="btn-red btn-full" style={{ marginBottom: 10 }} onClick={onSave}>
      Sauvegarder
    </button>
    ...
    <div style={{ display: "flex", gap: 8 }}>
      ...
      {!editResId && (
        <button className="btn-gray" style={{ flex: 1 }} onClick={onSaveAndAddAnother}>
          + Nouvelle réserve
        </button>
      )}
      {editResId && (
        <button className="btn-gray" style={{ flex: 1 }} onClick={onSave}>
          <I.Check /> Enregistrer
        </button>
      )}
    </div>
    ...
    {onViewAll && (
      <button
        ...
        style={{
          ...
          background: "white", color: "#D32F2F",
          border: "1.5px solid #D32F2F", borderRadius: 999,
