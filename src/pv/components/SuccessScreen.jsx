...
export default function SuccessScreen({ savedResult, reserveCount, onDownloadPDF, onViewPV, onGoHome, onShareEmail }) {
  const isEdit = savedResult?.isEdit;

  return (
    <div style={{ height: "100vh", overflow: "hidden", background: "white", display: "flex", flexDirection: "column" }}>

      {/* Scrollable content — fills space above fixed footer */}
      <div style={{ flex: 1, overflowY: "hidden", display: "flex", flexDirection: "column", alignItems: "center", padding: "24px 20px 60px" }}>
        ...
        <div style={{ position: "relative", marginBottom: 14, flexShrink: 0 }}>
          <div style={{
            ...
            boxShadow: "0 6px 22px rgba(211,47,47,0.38)",
          }}>
            <I.Shield />
          </div>
          <div style={{
        ...
        {/* Share by email */}
        <button onClick={onShareEmail} style={{
        ...
        {/* Quick view card */}
        <div onClick={onViewPV} style={{
          width: "100%", background: "white",
          borderRadius: 14, border: "1px solid #f0ece8",
          padding: "11px 14px",
        ...
        <div onClick={onGoHome} style={{
          ...
          textTransform: "uppercase", padding: "6px 0", flexShrink: 0,
        }}>
          RETOUR SUR LA LISTE DES PV
        </div>
      </div>

      {/* Accueil PSA footer — fixed */}
      <div style={{
