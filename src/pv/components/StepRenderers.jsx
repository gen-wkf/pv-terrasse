import { AGENCES, ETABLISSEMENTS, SURFACE_FIELDS, RELEVES_FIELDS, POINT_FIELDS } from "../schema.js";
import ToggleGroup from "./ToggleGroup.jsx";
import { I } from "../icons.jsx";
import Dictaphone from "./Dictaphone.jsx";

export const renderStep1 = (form, setForm, reserves, onOpenAddRes, renderReserveCard, onDeleteRes, onEditRes, onViewReserves) => (
  <div className="cnt fade-in">
    <div className="hint-box">
      <div className="hint-ico">
        <I.Doc />
      </div>
    ...
    <div className="field-wrap">
      ...
      <input
        className="inp"
        placeholder="Nom du projet ou référence client"
        value={form.chantier}
        onChange={(event) => setForm((current) => ({ ...current, chantier: event.target.value }))}
        autoComplete="off"
      />
    </div>
    ...
    <div className="field-wrap">
      ...
      <input
        className="inp"
        placeholder="Ex: Bâtiment B - Toiture"
        value={form.zone}
    ...
    <div style={{ background: "#f3f4f6", borderRadius: 16, padding: 18, border: "1.5px solid #f0ece8", boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
      <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 14 }}>
        <div style={{ width: 42, height: 42, background: "#D32F2F", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", color: "#ffffff", flexShrink: 0 }}>
          <I.Doc />
        </div>
        <div>
      ...
      {reserves.length === 0 ? (
        <button className="btn-red btn-full" onClick={onOpenAddRes}>
          + Ajouter une réserve
        </button>
      ) : (
        <button
...
export const renderStep2 = (etatSurface, setEtatSurface, step2Errors, onAddPhoto, onRemovePhoto, onPhotoMenu, onOpenGallery) => {
  const nonConformeExtras = (key, label) => {
    const photos = etatSurface[key + "Photos"] || [];
    const comment = etatSurface[key + "Comment"] || "";
    ...
    return (
      <div style={{ marginTop: 10, marginBottom: 4, padding: 14, background: "#fff8f8", borderRadius: 12, border: `1.5px solid ${photoErr || commentErr ? "#D32F2F" : "#fdd"}` }}>
        <div>
          ...
          {photos.length === 0 ? (
            <div
              ...
              onClick={() => onAddPhoto(key)}
              style={photoErr ? { borderColor: "#D32F2F", background: "#fff5f5" } : {}}
            >
            ...
            <>
              <div className="photo-grid">
                {shownPhotos.map((photo, i) => {
                  ...
                  return (
                    <div key={photo.id} className="ph-wrap" style={{ position: "relative" }}>
                      <img
                        ...
                        onClick={() => isLast
                          ...
                          : onPhotoMenu({
                              photo,
                              onReplace: () => {},
                              onUpdate: (url) => setEtatSurface((curr) => ({
                                ...curr,
                                [key + "Photos"]: (curr[key + "Photos"] || []).map((p) =>
                      ...
                      {isLast && (
                        <div
                          onClick={() => onOpenGallery(photos, label)}
                          style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", borderRadius: "inherit", zIndex: 1 }}
                        >
                ...
                {photos.length < 5 && (
                  <div className="ph-add-tile" onClick={() => onAddPhoto(key)} title="Ajouter">
                    <I.PlusCircle />
                  </div>
                )}
              </div>
              {hiddenCount > 0 && (
                <button
                  onClick={() => onOpenGallery(photos, label, key)}
                  style={{ marginTop: 10, width: "100%", background: "#fff0f0", border: "1.5px solid #fdd", borderRadius: 10, padding: "9px 0", color: "#D32F2F", fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "'DM Sans',sans-serif" }}
                >
        ...
        <div className="field-wrap" style={{ marginTop: 12 }}>
          <label className="lbl">
            Commentaire <span className="req-star">*</span>
          </label>
          <Dictaphone
            value={comment}
      ...
      <div className="sec">
        ...
        {SURFACE_FIELDS.map(([key, label], index) => (
          <div key={key}>
            <ToggleGroup
              label={label}
              value={etatSurface[key]}
              onChange={(value) => setEtatSurface((current) => ({
                ...current,
                [key]: value,
...
export const renderStep3 = (releves, setReleves) => (
  <div className="cnt fade-in">
    <div className="sec">
      <div className="sec-hdr">
...
export const renderStep4 = (points, setPoints) => (
  <div className="cnt fade-in">
    <div className="sec">
      <div className="sec-hdr">
      ...
      <div style={{ padding: "0 0 4px" }}>
        ...
        <textarea
          ...
          value={points.observations}
          onChange={(event) => setPoints((current) => ({ ...current, observations: event.target.value }))}
        />
      </div>
    </div>
    <div style={{ height: 8 }} />
