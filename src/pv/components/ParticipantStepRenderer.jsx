          ...
          {!isSMAC && (
            <div className="field-wrap">
              ...
              <input
                ...
                onChange={(event) => onUpdateParticipant(participant.id, "titre", event.target.value)}
                autoComplete="off"
              />
            </div>
          )}
          ...
          {showNom && (
            <div className="field-wrap">
              ...
              <input
                ...
                onChange={(event) => {
                  onUpdateParticipant(participant.id, "nom", event.target.value);
                }}
                autoComplete="off"
              />
          ...
          {showSig && (
            <div>
              <SignaturePad
                ...
                onSign={(value) => {
                  onUpdateParticipant(participant.id, "signed", value);
                }}
                onCanvasReady={(canvas) => {
                  canvasElements.current[index] = canvas;
                }}
                extraAction={index === participants.length - 1 && participants.length < 4 ? (
                  <button
                    ...
                    title={`Ajouter un participant (${participants.length - 1}/3)`}
                  >
                    <I.AddPerson />
                  </button>
                ) : null}
    ...
    <div className="sec">
      <div className="inline-row">
        ...
        <div className="yn-group">
          <button
            className={`yn-btn ${participants[0]?.reception === "OUI" ? "active-oui" : ""}`}
            disabled
            style={{ cursor: "default", opacity: participants[0]?.reception === "OUI" ? 1 : 0.4 }}
          >OUI</button>
          <button
            className={`yn-btn ${participants[0]?.reception === "NON" ? "active-non" : ""}`}
            disabled
            style={{ cursor: "default", opacity: participants[0]?.reception === "NON" ? 1 : 0.4 }}
          >NON</button>
        </div>
      </div>
        ...
        <div className="inline-row">
          ...
          <div className="inline-val" style={{ background: "#f5f5f5", color: "#555", display: "flex", alignItems: "center" }}>
            {participants[0]?.miseEnConformite
              ? participants[0].miseEnConformite.split("-").reverse().join("/")
              : "—"}
          </div>
        </div>
      )}
      ...
      {participants[0]?.reception === "NON" && (
        <div className="inline-row">
          ...
          <input
            ...
            type="date"
            value={participants[0]?.reporterAu || ""}
            min={new Date().toISOString().split("T")[0]}
            onChange={(event) => onUpdateParticipant(participants[0].id, "reporterAu", event.target.value)}
          />
        </div>
      )}
...
export const renderStep6 = (editPvId, savedPV, onViewPDF, onGoHome, email, onSendEmail, isSending) => (
  <div className="cnt fade-in">
    ...
    {email && (
      <button
        ...
        style={{
          marginTop: 10,
          padding: "10px 16px",
          background: isSending ? "#555" : "#1a1a3e",
          color: "white",
          border: "none",
          borderRadius: 10,
          fontWeight: 700,
    ...
    <div className="back-link" onClick={onGoHome}>
