...
export default function AnnotationModal({ src, onSave, onClose }) {
  ...
  useEffect(() => {
    ...
    image.onload = () => {
      ...
      const maxHeight = window.innerHeight - 210; // viewport minus controls (~185px) + padding
      let w = maxWidth;
  ...
  const handleMove = (e) => {
    ...
    if (tool === "rect" && snapshotRef.current) {
    ...
    if (tool === "circle" && snapshotRef.current) {
      ctx.putImageData(snapshotRef.current, 0, 0);
    ...
    if (tool === "arrow" && snapshotRef.current) {
      ctx.putImageData(snapshotRef.current, 0, 0);
  ...
  const handleUp = (e) => { e.preventDefault(); setDrawing(false); };
    ...
    <div style={{ width: "100%", maxWidth: 420, background: "#111", display: "flex", flexDirection: "column", position: "relative" }}>
      ...
      <div style={{ background: "#1a1a1a", flexShrink: 0 }}>
          ...
          {TOOLS.map((t) => (
            <button
              ...
              style={{
                ...
                background: tool === t.key ? "#D32F2F" : "transparent",
                color: tool === t.key ? "white" : "#9ca3af",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 16px 10px", justifyContent: "center", borderTop: "1px solid #2a2a2a" }}>
          {COLORS.map((c) => (
            <button
              ...
              onClick={() => setColor(c)}
              style={{
                width: color === c ? 28 : 24, height: color === c ? 28 : 24,
                borderRadius: "50%", background: c, border: "none", cursor: "pointer",
                outline: color === c ? "2.5px solid white" : "2px solid transparent",
          ...
          {SIZES.map((s) => (
            <button
              ...
              onClick={() => setSize(s)}
              style={{
                width: s === 3 ? 10 : 16, height: s === 3 ? 10 : 16,
                borderRadius: "50%", border: "none", cursor: "pointer", flexShrink: 0,
                background: size === s ? "#D32F2F" : "#555",
                transition: "background 0.15s",
              }}
            />
          ))}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 16px 20px" }}>
          <button
            onClick={undo}
            style={{ width: 46, height: 46, borderRadius: "50%", border: "none", background: "#2a2a2a", color: "#9ca3af", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}
            title="Annuler"
          >
            <I.Undo />
          </button>
          <button
            onClick={clearAll}
            style={{ width: 46, height: 46, borderRadius: 10, border: "none", background: "#3a1010", color: "#D32F2F", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}
            title="Tout effacer"
          >
            <I.Trash />
          </button>
