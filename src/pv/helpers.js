...
const defaultPdfSection = (value = "SO") => value || "SO";
...
export const createInitialForm = () => ({ ...INITIAL_FORM });
export const createInitialEtatSurface = () => ({ ...INITIAL_ETAT_SURFACE });
export const createInitialReleves = () => ({ ...INITIAL_RELEVES });
export const createInitialPoints = () => ({ ...INITIAL_POINTS });

export const createParticipant = (overrides = {}) => ({
  id: uid(),
  ...INITIAL_PARTICIPANT,
  reporterAu: "",
  ...overrides,
...
export const createParticipants = () => [createParticipant()];

export function formatPVTimestamp(date = new Date()) {
...
export const createPdfDataFromPV = (pv) => ({
  ...
  etatSurface: pv.etatSurface || { reg1: "SO", prop1: "SO", reg2: "SO", prop2: "SO" },
  releves: pv.releves || {
    trous: "SO",
    remplissage: "SO",
    hauteur: "SO",
...
export const createPdfDataFromVersion = (version) => ({
  ...
  etatSurface: version.etatSurface || { reg1: "SO", prop1: "SO", reg2: "SO", prop2: "SO" },
  releves: version.releves || {
    trous: "SO",
    remplissage: "SO",
    hauteur: "SO",
    profondeur: "SO",
...
export function downloadPDF(pvData) {
  ...
  const reserveHtml = reserves.map((reserve, i) => {
    const locPhoto = reserve.localisationPhoto?.url
      ...
      : "";
    const photos = (reserve.photos || []).length > 0
...
<div style="font-family:Arial,Helvetica,sans-serif;color:#1a1a1a;background:#f5f5f5;padding:24px;box-sizing:border-box">
  ...
  ${section("État de Surface",
    `<table style="width:100%;border-collapse:collapse">${SURFACE_FIELDS.map(([key, label]) => {
      const val = etatSurface[key];
      const photos = val === "Non Conforme" ? (etatSurface[key + "Photos"] || []) : [];
      const comment = val === "Non Conforme" ? (etatSurface[key + "Comment"] || "") : "";
...
export async function sendPVEmail(pvData, recipientEmail) {
  const { form, reserves, savedPV } = pvData;

  const reserveLines = reserves.length > 0
